const grid = document.getElementById('grid');
    ...
    function createRectangle(width, height) {
        ...
        rectangle.style.width = `${width * 40-3}px`;
        rectangle.style.height = `${height * 40-3}px`;
    ...
    function drag(event) {
        ...
        x = Math.max(0, Math.min(x, 20 * 40 - rectangle.offsetWidth)+25);
        y = Math.max(0, Math.min(y, 20 * 40 - rectangle.offsetHeight)+25);
    ...
    function snapToGrid() {
        ...
        rectangle.style.left = `${gridX * 40 }px`;
        rectangle.style.top = `${gridY * 40 }px`;
    ...
    function rotateRectangle() {
        if (!rectangle || rectangle.classList.contains("fixed")) return; // Если фигуры нет, ничего не делаем
