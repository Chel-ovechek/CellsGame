const grid = document.getElementById('grid');
    ...
    document.addEventListener('touchstart', (e) => {
        let target = e.target.closest('.rectangle');
        if (!target) return;
    ...
    function snapToGrid() {
        ...
        if (!canPlaceRectangle(gridX, gridY, currentWidth, currentHeight) && !canPlaceRectangle(gridX, gridY, currentWidth, currentWidth)) {
    ...
    function rotateRectangle() {
        ...
        rectangle.style.width = `${height-2}px`;
        rectangle.style.height = `${width-2}px`;
